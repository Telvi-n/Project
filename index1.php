...
if (!isset($_SESSION['opname']) && !isset($_SESSION['opname1'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['opname'];
  $filter1 = $_SESSION['opname1'];
...
<html class="no-js" lang="zxx">
    <head>
        ...
        <!-- Title -->
        <title>Ambulance Response System - Ambulance Operator Homepage</title>
                    ...
                    <div class="col-lg-12 col-md-12 col-12">
                        <div class="single-table">
                            ...
                            <div class="table-head">
                                <h4 class="title">List of My Ambulances</h4>
                            </div>
                            <!-- Table List -->
...
<?php
$sql = "SELECT `Ambulance_ID`, `Operator_ID`, `Hospital`, `ETA`, `Location`, `Image`, `Created_At` FROM `ambulance` WHERE `Operator_ID` = '$filter1'";
...
<?php
$sql = "SELECT `Emergency_ID`, `Patient_ID`, `Operator_ID`, `Ambulance_ID`, `Details`, `Location`, `Start_At`, `End_At`, `Status` FROM `emergency` WHERE `Operator_ID` = '$filter1'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to accept this request?')?window.location.href='insertion.inc.php?action=acceptE&id=<?php echo($row["Emergency_ID"]); ?>':true;" title='Accept Request'>Accept</button></td>
<?php
}else if($row["Status"] == 'Accepted'){
?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to complete this request?')?window.location.href='insertion.inc.php?action=completeE&id=<?php echo($row["Emergency_ID"]); ?>':true;" title='Complete Request'>Complete</button></td>
<?php
}else{
?>
